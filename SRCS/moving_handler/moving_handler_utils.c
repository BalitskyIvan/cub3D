...
void	turn_left(t_map *map, float delta)
{
	...
	{
		if (!is_collision_detected(delta * cos(map->player.alpha - PI / 2),
		delta * sin(map->player.alpha - PI / 2), map))
		{
...
void	turn_up(t_map *map, float delta)
{
	...
	{
		if (!is_collision_detected(delta * cos(map->player.alpha),
		delta * sin(map->player.alpha), map))
		{
